...
// 两种方式在核心意义上没有差别，都是创建Array这个类的而一个实例，但是在愈发上是有区别的
// 1、字面量创建方式传递进来什么，都是给数组每一项假如内容
// 2、构造函数创建方式new Array(10):创建一个长度为10的数组，数组中的每一项都是空
// new Array('10'): 如果值传递一个实参，并且实参不是数字，相当于把当前值作为数组的第一项存储进来
// new Array(10,20,30): 如果床底多个实参，不是设置长度，而是把传递的内容当作数组中的每一项存储起来
...
> 
> 4、向普通函数一样，代码自上而下执行：`this.xxx=xxx 这里操作都是给创建的这个对象增加属性名和属性值`
> 
...
> 当构造函数体中，我们自己手动设置了return（默认返回的是实例：对象类型值），return的是基本类型值，对最后返回的实例没有任何影响，但是如果返回的是引用数据类型只，会默认把返回的实例替换掉
